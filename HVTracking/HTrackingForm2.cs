...
{
    public partial class HTrackingForm2 : Form
    {
        double[,] A, IRQT;
        int N =>(int)Num.Value;
        ...
        {
            d = DCount.Value % 2 == 0 ? -1 : 1;
            int k = (int)((double)DCount.Value / 2.0 + 0.5) - N;
            k = k < 0 ? 0 : k;
            int sc = k * sizeof(double);
            if (d == 1)
            {
                ...
                richTextBox1.Text = "\r\n正侧角度：" + pang[k].ToString("F4") + "\r\nDC:" + cpy[0] + "\r\nCW0：" + cpw[0].ToString("F4") + "\r\nCW1：" + cp1.ToString("F4") + "\r\nDV:" + vpy[0] + "\r\nVW0：" + vpw[0].ToString("F4") + "\r\nVW1：" + vp1.ToString("F4");
            }
            else
            {
                ...
                pvnw = IRQT.Dot(pvny);
                double cn1 = 0, vn1 = 0;
                for (int i = 1; i < cnw.Length; i++)
                {
                    cn1 -= i * cnw[i] * Math.Pow(0.1, i - 1);
                    vn1 -= i * vnw[i] * Math.Pow(0.1, i - 1);
                }
                richTextBox1.Text = "\r\n负侧角度：" + nang[k].ToString("F4") + "\r\nDC:" + cny[0] + "\r\nCW0：" + cnw[0].ToString("F4") + "\r\nCW1：" + cn1.ToString("F4") + "\r\nDV:" + vny[0] + "\r\nVW0：" + vnw[0].ToString("F4") + "\r\nVW1：" + vn1.ToString("F4");
            }
        ...
        void ReadData()
        {
            ...
            using (StreamReader sr = new StreamReader(fileName))
            {
                //using (TextWriter tw = new StreamWriter("t2a.csv"))
                {
                    ...
                    DCount.Maximum = len - 1;
                    DCount.Minimum = 2 * N - 1;
                    DCount.Value = 2 * N - 1;
                    DCount_ValueChanged(null, null);
                }
            }
        ...
        private void DrawC(Graphics g, Rectangle rectangle)
        {
            ...
            {
                if(cpw!=null)
                pps[0] = new PointF((float)(pX - cpw[0] * tickX), margin + 1.5f * tickY);
                if(cnw != null)
                nps[0] = new PointF((float)(nX + cnw[0] * tickX), margin + tickY);
            }
            ...
            using (Pen pen = new Pen(Color.DarkRed, 2.5f), pen2 = new Pen(Color.YellowGreen, 2.5f), pen3 = new Pen(Color.IndianRed, 4.5f))
            {
                ...
                for (int i = 0; i < N; i++)
                {
                    ...
                    {
                        PointF cp = new PointF((float)(pX - cpy[i] * tickX), (float)(margin + (i + 2.5) * tickY));
                        g.DrawEllipse(pen, cp.X - 4, cp.Y - 4, 8, 8);
                        PointF cn = new PointF((float)(nX + cny[i] * tickX), (float)(margin + (i + 2.0) * tickY));
                        g.DrawEllipse(pen, cn.X - 4, cn.Y - 4, 8, 8);
                        ...
                        if (i == 0)
                        {
                            g.FillEllipse(new SolidBrush(Color.DeepPink), (float)(pps[0].X - 4), (float)(margin + 1.5f *tickY - 4), 8, 8);
                            g.FillEllipse(new SolidBrush(Color.DeepPink), (float)(nps[0].X - 4), (float)(margin +  tickY - 4), 8, 8);
                        ...
                        if(pncpy!=null)
                            ppps[i] = new PointF((float)(pX - pncpy[i] * tickX), (float)(margin + (i + 1.5) * tickY));
                        if(pncny!=null)
                            pnps[i] = new PointF((float)(nX + pncny[i] * tickX), (float)(margin + (i + 1.0) * tickY));
                        if(ncpy!=null)
                            pps[i + 1] = new PointF((float)(pX - ncpy[i] * tickX), (float)(margin + (i + 2.5) * tickY));
                        if(ncny!=null)
                            nps[i + 1] = new PointF((float)(nX + ncny[i] * tickX), (float)(margin + (i + 2.0) * tickY));
                        pRect = new Rectangle((int)(cp.X - labelw * 2.0), (int)(cp.Y - font.Height - 3), (int)labelw * 4, font.Height);
                        //g.DrawString((cpy[i]).ToString("F4"), font, sb, pRect, sf);
                        if (cpy[i] > 5)
                            g.DrawString((cpy[i]).ToString("F4"), font, new SolidBrush(Color.PaleVioletRed), pRect, sf);
        ...
        private void DrawV(Graphics g, Rectangle rectangle)
        {
            ...
            {
                if(vpw!=null)
                    pps[0] = new PointF((float)(pX + vpw[0] * tickX), margin + 1.5f * tickY);
                if (vnw != null)
                    nps[0] = new PointF((float)(nX - vnw[0] * tickX), margin + tickY);
            }
            ...
            using (Pen pen = new Pen(Color.DarkRed, 2.5f), pen2 = new Pen(Color.YellowGreen, 2.5f), pen3 = new Pen(Color.IndianRed, 4.5f))
            {
                ...
                for (int i = 0; i < N; i++)
                {
                    ...
                    {
                        PointF cp = new PointF((float)(pX + vpy[i] * tickX), (float)(margin + (i + 2.5) * tickY));
                        g.DrawEllipse(pen, cp.X - 4, cp.Y - 4, 8, 8);
                        PointF cn = new PointF((float)(nX - vny[i] * tickX), (float)(margin + (i + 2.0) * tickY));
                        g.DrawEllipse(pen, cn.X - 4, cn.Y - 4, 8, 8);
                        ps[i * 2] = cn;
                        ps[i * 2 + 1] = cp;
                        if (i == 0)
                        ...
                        if(nvpy!=null)
                        pps[i + 1] = new PointF((float)(pX + nvpy[i] * tickX), (float)(margin + (i + 2.5) * tickY));
                        if(nvny!=null)
                        nps[i + 1] = new PointF((float)(nX - nvny[i] * tickX), (float)(margin + (i + 2.0) * tickY));
                        if(pnvpy!=null)
                        ppps[i] = new PointF((float)(pX + pnvpy[i] * tickX), (float)(margin + (i + 1.5) * tickY));
                        if(pnvny!=null)
                        pnps[i] = new PointF((float)(nX - pnvny[i] * tickX), (float)(margin + (i + 1.0) * tickY));
