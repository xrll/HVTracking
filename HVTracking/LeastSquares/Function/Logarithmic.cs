...
{
    public struct Logarithmic : IFunction
    {
        ...
        public double[] Regression(double[] vx,double[] vy)
        {
            ...
            {
                tx = new double[] {1.0,Math.Log(vx[i])};
                ty = ty.Add(new double[] { vy[i], Math.Log(vx[i]) * vy[i] });
                ls = ls.Add(tx.Cov());
            }
...
//            tw = Matrix.InvertSsgjUnsafeU(ls).Dot(ty);
            ...
            if (det == 0)
                return null;
            tw[0] = (ls[1, 1] * ty[0] - ls[0, 1] * ty[1]) / det;
            tw[1] = (-ls[1, 0] * ty[0] + ls[0, 0] * ty[1]) / det;
            tw[2] = -1;
            return tw;
        }
        /// <summary>
        ...
        public double[] NLRegression(double[] vx, double[] vy, double[] w)
        {
            ...
            while (iter < maxIters)
            {
                ...
                for (int i = 0; i < vx.Length; i++)
                {
                    ...
                    g = new double[2] { 1, Math.Log(vx[i]) };
                    tj = tj.Add(g.Cov());
                    double dy = vy[i] - y;
                    mse += dy * dy;
